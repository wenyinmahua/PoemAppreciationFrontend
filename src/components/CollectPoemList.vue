    ...
    <a-comment  v-for="(poem, index) in poems" >
      ...
      <template #content>
        <p>
          ...
          <h1 class="poem-title" @click="console.log(poem.title)"><a style="color: black" @click="getPoemDetail(poem.title)"> 《{{ poem.title }}》</a></h1>
          <div class="poem-content" v-html="formatPoemContent(poem.content)"></div>
        </p>
      </template>
      <template #datetime>
        <a-tooltip >
          <a @click="console.log(poem.dynasty)">{{ poem.dynasty }}</a>
        </a-tooltip>
      </template>
...
const getPoem = (current:number,pageSize:number)=> myAxios.get("/user/liked/poem",{params:{
    current:current,
    pageSize : pageSize,
  }}).then((res)=>{
  if(res){
...
const getPoemDetail = (title:string)=>{
